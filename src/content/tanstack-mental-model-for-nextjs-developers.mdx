...
Next.js has a built-in Metadata API — you export `metadata` or `generateMetadata` from any page or layout. TanStack Start has its own built-in SEO path too, but it is route-first instead of file-convention-first. You define a `head` function on the route, and that route can generate titles, descriptions, Open Graph tags, canonical links, and structured data from params or loader data.
...
```tsx
// TanStack Start — route-level head API
...
So the better comparison is not "Next.js has metadata and TanStack Start makes you bring a head library." TanStack Start provides technical SEO primitives directly: SSR by default, route-level document head management, static prerendering, JSON-LD via route head scripts, robots.txt through static files or server routes, and built-in sitemap generation when prerendering/link crawling is enabled. It is still more explicit than Next.js, but SEO is a documented framework capability, not an afterthought.
