...
Add-ons cover auth (Clerk, Better Auth), database (Drizzle), UI (shadcn/ui, Tailwind), and TanStack Query — you pick what you need, nothing is bundled by default. Where `create-next-app` leaves auth/db/UI for you to wire up separately, TanStack's CLI makes those choices composable up front. A common stack — Better Auth + Drizzle + PostgreSQL (Neon/Supabase) + shadcn/ui + TanStack Query — emerges not because the framework mandates it, but because these tools share the same explicit, composable philosophy.
...
**The ecosystem is newer.** When you hit an edge case, you are more likely to be reading source code or GitHub issues than finding a polished answer someone else has already written.
...
## The 1:1 Translation Layer

If you keep one thing handy while porting your mental model over, make it this:
...
The two traps that aren't on that table, because they have no Next.js equivalent: loaders are isomorphic (never touch a DB, secret, or Node API directly in one — wrap it in `createServerFn`), and auth is two layers (`beforeLoad` guards the UX, server function middleware guards the data — you usually want both).
