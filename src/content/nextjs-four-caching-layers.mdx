...
<ScopeTable
  title="Where each cache layer lives"
  description="Three layers run on the server during rendering. Router Cache is the only client-side layer."
  columns={["Server", "Client"]}
  rows={[
    { label: "Request Memoization", values: [true, false] },
    { label: "Data Cache", values: [true, false] },
    { label: "Full Route Cache", values: [true, false] },
    { label: "Router Cache", values: [false, true] },
  ]}
/>
