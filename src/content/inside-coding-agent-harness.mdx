---
title: "It's Not Just a Loop: Inside a Coding Agent Harness"
summary: "A technical dissection of what runs inside coding agents: tool loops, sandboxing, compaction, memory, why the model is the smallest part, and what I learned
...
<BlogDiagram
  title="Coding agent harness architecture"
  description="Data flow from user input through routing, the LLM loop, tools, policy, sandbox, compaction, and persistence."
>
{`flowchart TB
  User[User Input] --> Router[Intent Router]
  Router --> Loop[LLM Loop]
  Project[Project Files / AGENTS.md] --> Loop
  Loop -->|tool calls| Tools[Tool Executor]
  Tools --> Policy[Permission Policy]
  Policy --> Sandbox[OS Sandbox]
...
<BlogDiagram
  title="Two-layer sandbox model"
  description="Permission policy checks intent before execution. The OS sandbox limits capability during execution."
>
{`flowchart TB
  Call[Model Tool Call] --> Policy[Permission Policy]
  Policy --> Decision{Allow / Ask / Deny}
  Decision -->|deny| Stop[Blocked]
  Decision -->|allow or approved| Exec[Execute Process]
  Exec --> Sandbox[OS Execution Sandbox]
  Sandbox --> Workspace[Writable Workspace]
  Sandbox --> Protected[Protected Paths]
  Sandbox --> Network[Network Restrictions]`}
</BlogDiagram>
...
<BlogDiagram
  ...
  description="Volatile in-session context, durable cross-session storage, and permanent project knowledge on disk."
>
{`flowchart TB
  subgraph Session["In-Session (volatile)"]
    Messages[Message Array]
    Messages --> Compaction[Compaction Summary]
    Compaction --> Messages
  end
  subgraph Restart["Cross-Session (persistent)"]
    SQLite[(SQLite Storage)]
    Messages --> SQLite
    SQLite --> Messages
  end
  subgraph Project["Project Knowledge (permanent)"]
...
- [Source Code for Archer](https://github.com/adarshaacharya/archer)
