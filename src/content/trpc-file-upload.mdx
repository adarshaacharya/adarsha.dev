---
title: "Upload Files to S3 in Next.js with tRPC: A Step-by-Step Guide"
type: Blog
...
Since trpc doesn't [multipart form data](https://github.com/trpc/trpc/issues/1937) directly we have to find out way to uplaod file to s3 without going to trpc server.
  ...
  getPreSignedUrl: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        // const [_attachment] = await db.insert(attachments).values({
        //   fullPath: path,
        //   mimeType: mimeType,
        //   extension,
        //   isPublic: true,
...
For the frontend, we'll use a file uploader component to handle file selection and upload. This example uses
[shadcn](https://ui.shadcn.com/) for UI components, but you can adapt it to your preferred library.
...
Finally, integrate the `ImageUploader` component into a form.
