...
<Callout emoji="💡">
The only way to access the private or public object is using the `key` of object. 
Most of the times the `key` value is stored in the database eg. in case of user table we store in avatar column the `key` alongside other data.
</Callout>
