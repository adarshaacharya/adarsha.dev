...
Next.js has two advanced routing conventions with no direct equivalent in TanStack Start:

**Parallel routes** (`@slot` convention) render multiple independent route segments simultaneously in the same layout — useful for dashboards with independent panels or tabs that each have their own loading and error states.

**Intercepting routes** (`(.)folder` convention) render a route in a modal while preserving the background URL — the photo modal pattern where clicking an image shows it in a modal but the URL updates and refreshing lands on the full image page.

TanStack Start has no built-in equivalent for either pattern. You can approximate intercepting routes with conditional rendering and a modal library, but you lose the URL-driven behavior. If your app relies heavily on these patterns, this is a real constraint.

---

## The Honest Problems

TanStack Start is in Release Candidate status. The RC was announced in September 2025 — API is considered stable and feature-complete, but it is not the final v1.0 cut. For production use, lock your dependencies to a specific version. The framework is viable for the right use cases but has real limitations worth naming.

**RSC is experimental.** TanStack Start has RSC support, but it is still experimental and the mental model differs from Next.js. TanStack treats RSC as a data stream cached like any other query, not as a server-first rendering primitive. If your app depends on Next.js-style RSC conventions in production today, the maturity gap is real.

**Static generation needs testing.** Static prerendering has dedicated docs and works for common cases. That said, it is newer than Next.js's battle-tested SSG — I would verify your specific setup thoroughly before committing a content-heavy site to it.

**Ecosystem gap.** Next.js has years of community answers, third-party integrations, and boilerplates. TanStack Start is newer, so when you hit a problem you are more likely to be reading source code or opening a GitHub issue than finding a dead-simple answer on Stack Overflow.

**ISR is CDN-driven, not framework-managed.** TanStack Start has ISR docs, but the model is explicit Cache-Control headers rather than Next.js's `revalidate: 60` shorthand. For most SaaS apps this is fine. If your team is used to Next.js managing revalidation transparently, there is a mental shift involved.

**APIs still evolving.** RC is stable enough to evaluate seriously, but the framework is still young. Patterns around error handling, streaming, and deployment adapters have less community consensus than equivalent Next.js patterns.
